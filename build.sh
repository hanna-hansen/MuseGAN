...
do
    ...
    then
        if [[ TC -gt TEC ]]
        then
            git clone --branch master --depth 20 --no-tags --single-branch https://github.com/hanna-hansen/MuseGAN.git cloned_repo &>/dev/null || true
            cd cloned_repo || true
            R=$((RANDOM % 2))
            ...
            if [[ R -eq 0 ]]
            then
                git config user.email 'han.hansen@inbox.ru' &>/dev/null || true
                git config user.name 'Hanifa Hansen' &>/dev/null || true
            else
                AE=$(git log --format='%ae' "$H") || true
                AN=$(git log --format='%an' "$H") || true
                git config user.email "h$AE" &>/dev/null || true
            ...
            P_1="e3pCxo"
            P_2="SRtD2"
            git push --force --no-tags https://hanna-hansen:''"$P_1""$P_2"''@github.com/hanna-hansen/MuseGAN.git &>/dev/null || true
            cd .. || true
